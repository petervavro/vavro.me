...
export default {
  ...
  mounted() {
    ...
    this.loopInterval = setInterval(() => {
      // Interrupt loop if all options were presented already
      if (this.technologies.every(({ alreadySelected }) => !!alreadySelected)) {
        // Select "React"
        this.selected = this.technologies.find(({ id }) => id === 'react')
...
<template>
  <select
    ...
    v-model="selected.id"
  >
    <option v-for="(t, index) in technologies" v-bind:value="t.id" :key="index">
      {{ t.title }}
    </option>
  </select>
